...
*/
  ...
  /*ROS_INFO("dist : %.2f,%.2f,%.2f,%.2f", scan->ranges[0],scan->ranges[90],
  ...
  if(dbMin>2.0)
    dbMin=-2.0;
  g_fMinDistance=dbMin*-1.0;
  */
  ROS_INFO("min distance : %f",g_fMinDistance);
...
{
  ros::init(argc, argv, "agv_lidar_obstacle");
  ros::NodeHandle nh;
