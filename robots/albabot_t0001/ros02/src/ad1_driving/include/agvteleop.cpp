...
AGVTeleop::AGVTeleop(uint8_t joyType)
{
  //nh_=*nodehandle;
...
void AGVTeleop::InitRobot(ros::Publisher *pubCmd, uint8_t opmode,
                          float agvVelocity, float kp, float ki, float kd,
                          float gearratio, float w2wDist, float acc)
{
  ...
  //감속비
  float_data.fData=gearratio;
  msg.dlc=6;
  msg.data.push_back(0x00);
  msg.data.push_back(0x45);
  msg.data.push_back(float_data.u8Data[0]);
  msg.data.push_back(float_data.u8Data[1]);
  msg.data.push_back(float_data.u8Data[2]);
  msg.data.push_back(float_data.u8Data[3]);
  ...
  //축간 거리
  float_data.fData=w2wDist;
  msg.dlc=6;
  msg.data.push_back(0x00);
  msg.data.push_back(0x46);
  msg.data.push_back(float_data.u8Data[0]);
  msg.data.push_back(float_data.u8Data[1]);
  msg.data.push_back(float_data.u8Data[2]);
  msg.data.push_back(float_data.u8Data[3]);
  ...
  //acc
  float_data.fData=acc;
  msg.dlc=6;
  msg.data.push_back(0x00);
  msg.data.push_back(0x42);
  msg.data.push_back(float_data.u8Data[0]);
  msg.data.push_back(float_data.u8Data[1]);
  msg.data.push_back(float_data.u8Data[2]);
  msg.data.push_back(float_data.u8Data[3]);
...
void AGVTeleop::JoyCB(const sensor_msgs::Joy::ConstPtr& _joy)
{
  ...
  if(m_u8JoyType==0)
  {
    ...
    m_u8NextBtn=_joy->buttons[1];
    m_u8TurnBtn=_joy->buttons[0];
    m_u8LeftBtn=_joy->buttons[2];
    m_u8RightBtn=_joy->buttons[3];
    m_u8CancelBtn=_joy->buttons[5];
...
uint8_t AGVTeleop::CheckButton(void)
{
  ...
  else if(m_u8PreCancelBtn==1 && m_u8CancelBtn==0)
  {
    u8Status=0xff;
    ROS_INFO("Cancel: OK");
  }
  ...
  m_u8PreCancelBtn=m_u8CancelBtn;
