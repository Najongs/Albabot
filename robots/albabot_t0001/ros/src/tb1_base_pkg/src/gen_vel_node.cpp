...
std_msgs::Float32MultiArray GetVelocity()
{
  ...
  ROS_INFO("vel : %f, %f(%f, %f)",vel1,vel2, _v, _w);
...
void CmdVelCB(const geometry_msgs::Twist::ConstPtr& value)
{
    ...
    float vel1,vel2;
    float scale=0.3;
    vel1=-1.0*(v-scale*w)*(3.3/3.3);
    vel2=-1.0*(v+scale*w)*(3.3/3.3);
    vel.data.push_back(vel1);
    vel.data.push_back(vel2);
    g_pubVel.publish(vel);
    ROS_INFO("vel(%f, %f), motor(%f, %f)", v,w,vel1,vel2);
    */
}
