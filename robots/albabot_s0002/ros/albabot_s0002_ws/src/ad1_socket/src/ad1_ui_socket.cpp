...
void GetParam(ros::NodeHandle *pNh)
{
  ...
  pNh->param<int>("port",nTemp,3456);
  g_nPortNum=nTemp;

}


// callback for robot_info
// 로봇제어용 프로그램의 요청에 의한 응답용 정보를 보관함.
...
// 로봇의 설정값을 보내기 위한 메시지 생성
void SetRobotConfig(albabot_msgs::CanMsg *pMsg, uint8_t cmd, float val)
{
  ...
  pMsg->dlc=6;
  pMsg->id=0;
  pMsg->data.push_back(0x00);
  pMsg->data.push_back(cmd);
  pMsg->data.push_back(floatData.u8Data[0]);
  pMsg->data.push_back(floatData.u8Data[1]);
  pMsg->data.push_back(floatData.u8Data[2]);
  pMsg->data.push_back(floatData.u8Data[3]);
...
{
  ros::init(argc, argv, "ad1_ui_socket");
  ros::NodeHandle nh;
  bool blnIsConnected=false;

  GetParam(&nh);

  ros::Subscriber robotInfo_pub_=nh.subscribe("robot_info",100, RobotInfoCB);
  ...
  SetRobotConfig(&msg, 0x45,50.0);
  g_command_pub.publish(msg);
  msg.data.clear();
  sleep(1);
  /*SetRobotConfig(&msg, 0x45,30.0);
  g_command_pub.publish(msg);
  msg.data.clear();
  sleep(1);*/
      ...
      if((length == (u8DLC*2+11)) && pParsingBuf[u8DLC*2+10]=='\r')
      {
        if(u8DLC!=0)
        {
          for(int i=0;i<u8DLC;i++)
          {
            ...
            pMsg->data.push_back(_get8U(pTemp));
